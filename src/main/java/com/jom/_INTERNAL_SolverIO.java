...
class _INTERNAL_SolverIO
{
	...
	_INTERNAL_SolverIO(OptimizationProblem model, Expression objectiveFunction, boolean toMinimize, HashMap<String, _INTERNAL_DecisionVariableArray> decisionVariables, HashMap<String, _INTERNAL_ConstraintArray> constraints , HashMap<String,DoubleMatrixND> initialSolution)
	{
		...
		for (Entry<String, _INTERNAL_ConstraintArray> entry : constraints.entrySet())
		{
			...
			DoubleMatrix1D lowerBound = (constraintSet.getConnector().equals(_INTERNAL_ConstraintArray.SYMBOL_LESSEQUAL)) ? DoubleFactory1D.dense.make(numConstraints, -Double.MAX_VALUE) : DoubleFactory1D.dense.make(numConstraints, 0);
			DoubleMatrix1D upperBound = (constraintSet.getConnector().equals(_INTERNAL_ConstraintArray.SYMBOL_GREATEREQUAL)) ? DoubleFactory1D.dense.make(numConstraints, Double.MAX_VALUE) : DoubleFactory1D.dense.make(numConstraints, 0);
		...
		if (in.lhsMinusRhsAccumulatedConstraint == null) { if (in.numConstraints != 0) throw new JOMException ("Unexpected error"); }
...
//			s += DoubleFactory2D.dense.make(coefs.toArray()) + "\n";
			...
			for (int contCell = 0 ; contCell < in.lhsMinusRhsAccumulatedConstraint.getNumScalarExpressions() ; contCell ++)
				s = s + in.constraintLowerBound.get(contCell) + " <= " + coefs.toStringLinearScalarExpression(contCell) + " <= " + in.constraintUpperBound.get(contCell) + "\n";
		}
		...
		s += "        multiplierOfLowerBoundConstraintToPrimalVariables: " + ((out.multiplierOfLowerBoundConstraintToPrimalVariables != null)? Arrays.toString(out.multiplierOfLowerBoundConstraintToPrimalVariables.toArray()):"null") + "\n";
		s += "        multiplierOfUpperBoundConstraintToPrimalVariables: " + ((out.multiplierOfUpperBoundConstraintToPrimalVariables != null)? Arrays.toString(out.multiplierOfUpperBoundConstraintToPrimalVariables.toArray()):"null") + "\n";
		s += "        primalValuePerConstraint: " + Arrays.toString(out.primalValuePerConstraint.toArray()) + "\n";
		s += "        multiplierOfConstraint: " + ((out.multiplierOfConstraint != null)? Arrays.toString(out.multiplierOfConstraint.toArray()):"null") + "\n";
	...
	void checkFeasibility ()
	{
		...
		{
			if ((out.primalSolution.get(cont) < in.primalSolutionLowerBound.get(cont) - ABS_TOLERANCE)) throw new JOMException ("Check feasibility error. Decision variable ID " + cont + " has value: " + out.primalSolution.get(cont) + ", which is outside the feasibility limits [" + in.primalSolutionLowerBound.get(cont) + "," + in.primalSolutionUpperBound.get(cont) + "]");
			if ((out.primalSolution.get(cont) > in.primalSolutionUpperBound.get(cont) + ABS_TOLERANCE)) throw new JOMException ("Check feasibility error. Decision variable ID " + cont + " has value: " + out.primalSolution.get(cont) + ", which is outside the feasibility limits [" + in.primalSolutionLowerBound.get(cont) + "," + in.primalSolutionUpperBound.get(cont) + "]");
		}
		/* Check feasibility of the solution --> constraint limits */
		DoubleMatrix1D c_constraintValues = (in.numConstraints == 0)? null : in.lhsMinusRhsAccumulatedConstraint.evaluate_internal(out.primalSolution.toArray()).elements();
		if (in.numConstraints != 0)
		{
			for (int cont = 0; cont < in.numConstraints; cont++)
			{
				if ((c_constraintValues.get(cont) < in.constraintLowerBound.get(cont) - ABS_TOLERANCE)) throw new JOMException ("Check feasibility error. Constraint number " + cont + " has a value (left hand side minus right hand side): " + c_constraintValues.get(cont) + ", which is outside the feasibility limits [" + in.constraintLowerBound.get(cont) + "," + in.constraintUpperBound.get(cont) + "]");
				if ((c_constraintValues.get(cont) > in.constraintUpperBound.get(cont) + ABS_TOLERANCE)) throw new JOMException ("Check feasibility error. Constraint number " + cont + " has a value (left hand side minus right hand side): " + c_constraintValues.get(cont) + ", which is outside the feasibility limits [" + in.constraintLowerBound.get(cont) + "," + in.constraintUpperBound.get(cont) + "]");
			}
		}
		...
		{
			System.out.println("Warning error. The primal cost returned by the solver does not match the objetive function evaluated in the returned optimal point. Solver primal cost: " + out.primalCost + ". Evaluated point: " + c_primalCostFromPrimalSolution + ". This happens sometimes in IPOPT solver, so what I do here is set the primal cost as the one coming from evaluating the primal solution returned, and drop the optimum cost computed by IPOPT");
			out.primalCost = c_primalCostFromPrimalSolution;
			//throw new JOMException ("Check error. The primal cost returned by the solver does not match the objetive function evaluated in the returned optimal point. Solver primal cost: " + out.primalCost + ". Evaluated point: " + c_primalCost);
		}
	}
	...
	void checkOptimality()
	{
		...
		if (in.numConstraints != 0)
		{
			for (int cont = 0; cont < in.numConstraints; cont++)
			{
				...
				if ((mult > ABS_TOLERANCE) && (slack_inf > ABS_TOLERANCE) && (slack_sup > ABS_TOLERANCE)) throw new JOMException ("Check error. Complementary slackness conditions in constrain number " + cont + " are not satisfied. Multiplier: " + mult + ". Slack with lower bound: " + slack_inf + ". Slack with upper bound: " + slack_sup);
			}
		}
		...
		for (int cont = 0; cont < in.numDecVariables; cont++)
		{
			...
			if ((mult_inf > ABS_TOLERANCE) && (slack_inf > ABS_TOLERANCE)) throw new JOMException ("Check error. Complementary slackness condition in lower bound constraint to decision variable number " + cont + " is not satisfied. Decision variable value: " + out.primalSolution.get(cont) + ". Lower bound: " + in.primalSolutionLowerBound.get(cont) + ". Slackness: " + slack_inf + ". Multiplier: " + mult_inf);
			if ((mult_sup > ABS_TOLERANCE) && (slack_sup > ABS_TOLERANCE)) throw new JOMException ("Check error. Complementary slackness condition in upper bound constraint to decision variable number " + cont + " is not satisfied. Decision variable value: " + out.primalSolution.get(cont) + ". Upper bound: " + in.primalSolutionUpperBound.get(cont) + ". Slackness: " + slack_sup + ". Multiplier: " + mult_sup);
		}
	...
	{
		if (this.in.lhsMinusRhsAccumulatedConstraint.isLinear()) return this.in.lhsMinusRhsAccumulatedConstraint.affineExp.toStringLinearScalarExpression(contraintId); 
