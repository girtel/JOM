 ...
 * 
 * @author Pablo Pavï¿½n Mariï¿½o, Technical University of Cartagena (Spain)
 * @version 0.1
 * 12/02/2013
 */
package com.jom;
...
class _INTERNAL_AffineExpressionCoefs
{
	...
	private class Cell 
	{ 
		double constantValue; LinkedHashMap<Integer,Double> lCoefs;  
		Cell () { this.constantValue = 0; this.lCoefs = null; } 
		Cell (double val) { this.constantValue = val; this.lCoefs = null; }
		Cell (LinkedHashMap<Integer,Double> lCoefs) { this.constantValue = 0 ; this.lCoefs = lCoefs; }
		Cell (double constantValue , LinkedHashMap<Integer,Double> lCoefs) { this.constantValue = constantValue ; this.lCoefs = lCoefs; }
		double evaluate (double [] dvValues) { double res = this.constantValue; if (this.lCoefs == null) return res; for (Entry<Integer,Double> lc : this.lCoefs.entrySet()) res += lc.getValue() * dvValues [lc.getKey()]; return res; }
		Cell sum (Cell c) 
		{ this.constantValue += c.constantValue; if (c.lCoefs == null) return this;  if (this.lCoefs == null) { this.lCoefs = c.lCoefs; return this; } 
			for (Entry<Integer,Double> lcC : c.lCoefs.entrySet()) { Double thisLcVal = this.lCoefs.get(lcC.getKey()); if (thisLcVal == null) this.lCoefs.put(lcC.getKey(), lcC.getValue()); else this.lCoefs.put(lcC.getKey(), thisLcVal + lcC.getValue()); }
			return this;
		}
		Cell substract (Cell c) 
		{ this.constantValue -= c.constantValue; if (c.lCoefs == null) return this;  if (this.lCoefs == null) { this.lCoefs = new LinkedHashMap<Integer,Double> (); } 
			for (Entry<Integer,Double> lcC : c.lCoefs.entrySet()) { Double thisLcVal = this.lCoefs.get(lcC.getKey()); if (thisLcVal == null) this.lCoefs.put(lcC.getKey(), -lcC.getValue()); else this.lCoefs.put(lcC.getKey(), thisLcVal - lcC.getValue()); }
			return this;
		}
		Cell multiplyConstant (double c) 
		{
			if (c == 0) { this.constantValue = 0; this.lCoefs = null; return this; }
		...
		{
			if (c == null) { this.constantValue = 0; this.lCoefs = null; return this; }
			if ((c.lCoefs != null) && (this.lCoefs != null)) throw new JOMException ("Affine expression operation error: multiplication of two non constant linear expressions is not a linear expression");
			if (c.lCoefs == null) { multiplyConstant (c.constantValue); return this; } // c is a constant
		...
		Cell deepCopy () { if (this.lCoefs == null) return new Cell (this.constantValue); else { LinkedHashMap<Integer,Double> res = new LinkedHashMap<Integer,Double> (); for (Entry<Integer,Double> e : this.lCoefs.entrySet()) res.put(e.getKey(), e.getValue()); return new Cell (this.constantValue ,  res); } }
		public String toString () { String s = "" + this.constantValue; if (this.lCoefs != null) for (Entry<Integer,Double> e : this.lCoefs.entrySet()) s = s + " + " + e.getValue() + "*" + model.getVarInfo(e.getKey()).getName() + "(" + (e.getKey() - model.getVarInfo(e.getKey()).getFirstVarId()) + ")";  return s;  };
	} 
	...
	_INTERNAL_AffineExpressionCoefs (OptimizationProblem model , IntMatrixND varIds)
	{
		...
		for (int cellCont = 0 ; cellCont < numCells ; cellCont ++)
		{
			...
			LinkedHashMap<Integer,Double> v = new LinkedHashMap<Integer,Double> (); v.put(dv , 1.0);
	...
	LinkedHashMap<Integer,Double> getCellNonZeroLinearCoefs (int cellIndex) { Cell c = this.linearCoefs.get(cellIndex); return (c == null)? null : c.lCoefs; }
	
	double getCellConstantCoef (int cellIndex) { if (this.linearCoefs == null) throw new JOMException ("Affine expression operation error: Unexpected error");   Cell c = this.linearCoefs.get(cellIndex); return (c == null)? 0 : c.constantValue; }
	...
	int numNonZerosLinearCoefs (int cellIndex) { Cell c = this.linearCoefs.get(cellIndex); return (c == null)? 0 : (c.lCoefs == null)? 0 : c.lCoefs.size (); }
	int numNonZerosLinearCoefs () { int res = 0; for (int c = 0 ; c < this.numCells ; c ++) res += numNonZerosLinearCoefs (c); return res; }
		...
		for (Entry<Integer,Cell> e : this.linearCoefs.entrySet())
		{
			...
			if (c.lCoefs != null)
				for (Entry<Integer,Double> lc : c.lCoefs.entrySet()) 
					{ rowsArray [counter] = cellId; colsArray [counter] = lc.getKey(); valsArray [counter ++] = lc.getValue(); }
		}
		rows [0] = rowsArray;
		cols [0] = colsArray;
		vals [0] = valsArray;
	...
	{
		if ((this.numDim != 2) || (a.getNumDim() != 2)) throw new JOMException ("Affine expression operation error: Append rows function only applies to 2-D expressions, in the form of column vector");
		if  ((this.size[1] != 1) || (a.getSize() [1] != 1)) throw new JOMException ("Affine expression operation error: Append rows function only applies to 2-D expressions, in the form of column vector");
	...
	{
		if (this.getNumDim() != 2) throw new JOMException ("Affine expression operation error: Diag function is for 2-D expressions in the form of row or column vector");
		if ((this.getSize () [0] != 1) && (this.getSize () [1] != 1)) throw new JOMException ("Affine expression operation error: Diag function is for 2-D expressions in the form of row or column vector");
	...
	{
		if (this.getNumDim() != 2) throw new JOMException ("Affine expression operation error: Ones and zeros functions should receive a row or column vector of constants indicanting the array size");
		if ((this.getSize () [0] != 1) && (this.getSize () [1] != 1)) throw new JOMException ("Affine expression operation error: Ones and zeros functionsshould receive a row or column vector of constants indicanting the array size");
		if (!this.isConstant()) throw new JOMException ("Affine expression operation error: Ones and zeros functions should receive a row or column vector of constants indicanting the array size");

		int [] size_i = new int [this.getNumScalarExpressions()]; for (int cont = 0; cont < size_i.length ; cont ++) size_i [cont] = (int) this.linearCoefs.get(cont).constantValue; 
		this.resize(size_i);
		this.linearCoefs = new LinkedHashMap <Integer,Cell> (); 
	...
	static _INTERNAL_AffineExpressionCoefs operator_multiply (_INTERNAL_AffineExpressionCoefs a , _INTERNAL_AffineExpressionCoefs b)
	{
		...
		final int [] resSize = Arrays.copyOf(b.size, b.size.length); resSize [0] = aRows;
...
//	}
//
//	static _INTERNAL_AffineExpressionCoefs function_matProdOld (_INTERNAL_AffineExpressionCoefs A , _INTERNAL_AffineExpressionCoefs B , boolean dimensionsApplyToA , int dimensionRows0Index , int dimensionColumns0Index)
//	{
//		if (!A.isConstant() && !B.isConstant()) throw new JOMException ("Affine expression operation error: Unexpected error");
//		//if ((A.numDim <= 2) && (B.getNumDim() <= 2)) throw new JOMException ("Affine expression operation error: matProd operation is applicable if at least a matrix is larger than 2D");
...
//	}
//
//	static _INTERNAL_AffineExpressionCoefs function_matProd (_INTERNAL_AffineExpressionCoefs A , _INTERNAL_AffineExpressionCoefs B , boolean dimensionsApplyToA , int dimensionRows0Index , int dimensionColumns0Index)
//	{
//		if (!A.isConstant() && !B.isConstant()) throw new JOMException ("Affine expression operation error: Unexpected error");
//		//if ((A.numDim <= 2) && (B.getNumDim() <= 2)) throw new JOMException ("Affine expression operation error: matProd operation is applicable if at least a matrix is larger than 2D");
//		LinkedHashMap<Integer,Cell> globalRes_linearCoefs = new LinkedHashMap<Integer,Cell> ();
...
//
	...
	_INTERNAL_AffineExpressionCoefs operator_transpose ()
	{
		...
		LinkedHashMap <Integer,Cell> newLinearCoefs = new LinkedHashMap <Integer,Cell> (); // first key is the cell index, second key is the variable index
	...
	_INTERNAL_AffineExpressionCoefs function_permute (int [] permutedDimensionsIndex0)
	{
		...
		final int [] newSize = new int [permutedDimensionsIndex0.length]; for (int dim = 0; dim < newSize.length ; dim ++) newSize [dim] = this.size [permutedDimensionsIndex0 [dim]];
		this.resize (newSize);
		LinkedHashMap <Integer,Cell> newLinearCoefs = new LinkedHashMap <Integer,Cell> (); // first key is the cell index, second key is the variable index
		...
		for (Entry<Integer,Cell> cellEntry : this.linearCoefs.entrySet())
		{
			...
			final IntMatrix1D newSubindexes = IntFactory1D.dense.make(this.numDim); for (int dim = 0; dim < newSize.length ; dim ++) newSubindexes.set(dim , oldSubindexes.get(permutedDimensionsIndex0 [dim]));
	...
	{
		if ((this.numDim  != 2) || (a.getNumDim() != 2)) throw new JOMException ("Affine expression operation error: Append columns function only applies to 2-D expressions");
		if (this.size [0] != a.getSize() [0]) throw new JOMException ("Affine expression operation error: Append columns function only applies to 2-D expressions, with the same number of rows");
	...
	{
		if ((this.numDim  != 2) || (a.getNumDim() != 2)) throw new JOMException ("Affine expression operation error: Append rows function only applies to 2-D expressions");
		if (this.size [1] != a.getSize() [1]) throw new JOMException ("Affine expression operation error: Append rows function only applies to 2-D expressions, with the same number of columns");
	...
	{
		if (!Arrays.equals(this.size, a.getSize())) throw new JOMException ("Affine expression operation error: 2e2 multiply function. Wrong size of array");
		...
		for (Entry<Integer,Cell> cellEntry : this.linearCoefs.entrySet())
		{
			...
			if (cell_a == null) this_cellIdsToRemove.add(cellId); else cell_this.multiply(cell_a);
		}
	...
	{
		if (!Arrays.equals(this.size, a.getSize())) throw new JOMException ("Affine expression operation error: 2e2 divide function. Wrong size of array");
		if (!a.isConstant()) throw new JOMException ("Affine expression operation error: 2e2 divide function. Denominator must be a constant to produce a linear expression");
	...
	_INTERNAL_AffineExpressionCoefs operator_plus (_INTERNAL_AffineExpressionCoefs a) // antiguo plusMinus
	{
		for (Entry<Integer,Cell> cellEntry : a.linearCoefs.entrySet())
		{
			...
			if (cell_this == null) this.linearCoefs.put(cellId, cell_a); else cell_this.sum(cell_a);
		}
		return this;
	}
	...
	_INTERNAL_AffineExpressionCoefs operator_substract (_INTERNAL_AffineExpressionCoefs a) // antiguo plusMinus
	{
		for (Entry<Integer,Cell> cellEntry : a.linearCoefs.entrySet())
		{
			...
			Cell cell_a = cellEntry.getValue();
			Cell cell_this = this.linearCoefs.get(cellId);
			if (cell_this == null) this.linearCoefs.put(cellId, cell_a.neg()); else cell_this.substract(cell_a);
		}
		return this;
	}
	...
	{
		if (this.numCells  != 1) throw new JOMException ("Affine expression operation error: Repeat expression function. Repeat function is for scalar expressions");
	...
	{
		if (this.numCells  != 1) throw new JOMException ("Affine expression operation error: Repeat diagonal expression function. Repeat function is for scalar expressions");
		Cell c = this.linearCoefs.get(0);
	...
	final _INTERNAL_AffineExpressionCoefs reshape (int [] newSize) { if (prod (newSize) != prod (this.size)) throw new JOMException ("Affine expression operation error: Reshape expression function. Wrong array size"); this.resize (Arrays.copyOf(newSize ,  newSize.length)); return this; }
	
	private static int prod (int [] array) { int res = 1; for (int num : array) res *= num; return res; }

	static int [] toArrayInt (ArrayList<Integer> a) { int [] res = new int [a.size()]; int counter = 0; for (int val : a) res [counter ++] = val; return res; }
	static double [] toArrayDouble (ArrayList<Double> a) { double [] res = new double [a.size()]; int counter = 0; for (double val : a) res [counter ++] = val; return res; }
	boolean isConstant ()
	{
		for (Cell c : this.linearCoefs.values())
	...
	static ArrayList<Integer> newAggregatedListNonOrderNonRepeatedNonZeros (final ArrayList<Integer> a  , final ArrayList<Integer> b , final int maxNum)
	{
		...
		for (int val : a)
			if (!alreadyInserted [val]) { res.add(val); alreadyInserted [val] = true; }
		for (int val : b)
			if (!alreadyInserted [val]) { res.add(val); alreadyInserted [val] = true; }
	...
	private void resize (int [] size) { this.size = size; this.numDim = size.length; this.numCells = 1; for (int c : size) this.numCells *= c; }
