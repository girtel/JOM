...
class _FUNCTION_ACOS extends Expression
{
	private final Expression a; 
	private final boolean isConstant;
	private final boolean isLinear;
	
	_FUNCTION_ACOS(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0 ; cont < data.length ; cont ++) data [cont] = Math.acos(data [cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		return new DoubleMatrixND (this.getSize () , aValue.assign(DoubleFunctions.acos));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return  DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.square).assign(DoubleFunctions.neg).assign(DoubleFunctions.plus(1)).assign(DoubleFunctions.sqrt).assign(DoubleFunctions.inv).assign(DoubleFunctions.neg)).zMult(a_jacobian, null);
	}
