...
class _FUNCTION_COS extends Expression
{
	private final Expression a; 
	private final boolean isConstant;
	private final boolean isLinear;
	
	_FUNCTION_COS(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0 ; cont < data.length ; cont ++) data [cont] = Math.cos(data [cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		return new DoubleMatrixND (this.getSize () , aValue.assign(DoubleFunctions.cos));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return  DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.sin).assign(DoubleFunctions.neg)).zMult(a_jacobian, null);
	}
	/* (non-Javadoc)
