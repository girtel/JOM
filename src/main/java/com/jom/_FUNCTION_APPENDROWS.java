...
class _FUNCTION_APPENDROWS extends Expression
{
	...
	_FUNCTION_APPENDROWS(OptimizationProblem model, Expression a, Expression b)
	{
		super(model);
		/* compute size */
		if ((a.getNumDim() != 2) || (b.getNumDim() != 2)) throw new JOMException("Function 'append rows': This function only applies to 2-D expressions, in the form of column vector");
		if ((a.getSize() [1] != 1) || (b.getSize() [1] != 1)) throw new JOMException("Function 'append rows': This function only applies to 2-D expressions, in the form of column vector");
