...
class _OPERATOR_APPENDROWS extends Expression
{
	...
	_OPERATOR_APPENDROWS(OptimizationProblem model, Expression a, Expression b)
	{
		...
		if ((a.getNumDim() != 2) || (b.getNumDim() != 2))
			throw new JOMException("Operator ';;' (append rows): This operator only applies to 2-D expressions");
		if (a.getSize()[1] != b.getSize()[1])
			throw new JOMException("Operator ';;' (append rows): This operator only applies to 2-D expressions, with the same number of columns");
		...
		this.permutationMatrix = (this.isLinear()) ?
				null :
				computePermutationMatrixDouble(a, b); // if linear do not store the expressions. This info is already in the linear coefs matrix
	}
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		DoubleMatrixND res = new DoubleMatrixND(this.getSize(), this.permutationMatrix.zMult(DoubleFactory1D.sparse.append(aValue.elements(), bValue
				.elements()), null));
		return res;
	}
