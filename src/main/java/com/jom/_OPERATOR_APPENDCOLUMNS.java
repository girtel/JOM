...
class _OPERATOR_APPENDCOLUMNS extends Expression
{
	...
	_OPERATOR_APPENDCOLUMNS(OptimizationProblem model, Expression a, Expression b)
	{
		...
		/* compute size */
		if ((a.getNumDim() != 2) || (b.getNumDim() != 2)) throw new JOMException("Operator ';' (append columns): This operator only applies to 2-D expressions");
		if (a.getSize() [0] != b.getSize() [0]) throw new JOMException("Operator ';' (append columns): This operator only applies to 2-D expressions, with the same number of rows");
