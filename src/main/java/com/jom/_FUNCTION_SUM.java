...
class _FUNCTION_SUM extends Expression
{
	...
	private DoubleMatrix2D sumOperationMatrix; // rows = num cells this (new) , columns = num cells a (old). 1 if new cell includes old cell as summand
	...
	private _FUNCTION_SUM(OptimizationProblem model, Expression a, int dimension)
	{
		...
		else if (a.getNumDim() == 2) { int [] newSize = Arrays.copyOf(a.getSize() , a.getSize().length); newSize [dimension] = 1; this.resize(newSize); }
		else 
		{
	...
	private static DoubleMatrix2D computeSumOperationMatrix (Expression a , int dimension)
	{
		...
		int numCellsPosteriorDimensions = (dimension== a.getNumDim() - 1) ? 0 : IntMatrixND.prod(a.getSize() , dimension + 1, a.getNumDim() - dimension - 1);
