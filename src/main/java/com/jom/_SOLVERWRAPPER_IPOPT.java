...
class _SOLVERWRAPPER_IPOPT
{
	...
	static String errorMessage(int code)
	{
		...
		case 1:
			return "Solved_To_Acceptable_Level. This indicates that the algorithm did not converge to the ``desired'' tolerances, but that it was able to obtain a point satisfying the ``acceptable'' tolerance level as specified by acceptable-* options. This may happen if the desired tolerances are too small for the current problem";
		case 2:
			return "Infeasible_Problem_Detected. The restoration phase converged to a point that is a minimizer for the constraint violation (in the -norm), but is not feasible for the original problem. This indicates that the problem may be infeasible (or at least that the algorithm is stuck at a locally infeasible point). The returned point (the minimizer of the constraint violation) might help you to find which constraint is causing the problem. If you believe that the NLP is feasible, it might help to start the optimization from a different point.";
		case 3:
			return "Search_Direction_Becomes_Too_Small. This indicates that IPOPT is calculating very small step sizes and making very little progress. This could happen if the problem has been solved to the best numerical accuracy possible given the current scaling.";
		case 4:
			return "Diverging_Iterates. This message is printed if the max-norm of the iterates becomes larger than the value of the option diverging_iterates_tol. This can happen if the problem is unbounded below and the iterates are diverging. ";
		case 5:
			return "User_Requested_Stop. This message is printed if the user call-back method intermediate_callback returned false";
		case 6:
			return "Feasible_Point_Found. This message is printed if the problem is ``square'' (i.e., it has as many equality constraints as free variables) and IPOPT found a feasible point.";
		case -1:
			return "Maximum_Iterations_Exceeded. This indicates that IPOPT has exceeded the maximum number of iterations as specified by the option max_iter";
		case -2:
			return "Restoration_Failed. This indicates that the restoration phase failed to find a feasible point that was acceptable to the filter line search for the original problem. This could happen if the problem is highly degenerate, does not satisfy the constraint qualification, or if your NLP code provides incorrect derivative information";
		case -3:
			return "Error_In_Step_Computation. This message is printed if IPOPT is unable to compute a search direction, despite several attempts to modify the iteration matrix. Usually, the value of the regularization parameter then becomes too large. One situation where this can happen is when values in the Hessian are invalid (NaN or Inf). You can check whether this is true by using the check_derivatives_for_naninf option. ";
		case -4:
			return "Maximum_CpuTime_Exceeded. ";
		case -10:
			return "Not_Enough_Degrees_Of_Freedom. This indicates that your problem, as specified, has too few degrees of freedom. This can happen if you have too many equality constraints, or if you fix too many variables (IPOPT removes fixed variables). ";
		case -11:
			return "Invalid_Problem_Definition. This indicates that there was an exception of some sort when building the IpoptProblem structure in the C or Fortran interface. Likely there is an error in your model or the main routine. ";
		case -12:
		...
		case -100:
			return "Unrecoverable_Exception. This indicates that IPOPT has thrown an exception that does not have an internal return code. See the specific message for details. ";
		case -101:
			return "NonIpopt_Exception_Thrown. An unknown exception was caught in IPOPT. This exception could have originated from your model or any linked in third party code. ";
		case -102:
			return "Insufficient_Memory. An error occurred while trying to allocate memory. The problem may be too large for your current memory and swap configuration. ";
		case -199:
	...
	void solve()
	{
		...
		LinkedHashMap<Integer,HashSet<Integer>> activeVarIds = (m == 0)? new LinkedHashMap<Integer,HashSet<Integer>> () : s.in.lhsMinusRhsAccumulatedConstraint.getActiveVarIds();

		int numNonZerosActiveVarIdsMatrix = 0; if (m > 0) for (HashSet<Integer> colIds : activeVarIds.values()) numNonZerosActiveVarIdsMatrix += colIds.size (); 
		...
		for (Entry<Integer,HashSet<Integer>> e : activeVarIds.entrySet())
			for (Integer dv : e.getValue())
				{ aux_rowsArrayActiveVarIds [counter] = e.getKey(); aux_colsArrayActiveVarIds [counter ++] = dv; }
		...
		if ((rowsArrayActiveVarIds.length != numActiveCoordinates) || (colsArrayActiveVarIds.length != numActiveCoordinates)) throw new JOMException("JOM - IPOPT interface. Unexpected error");
		...
		final int[] indexesOfGradientActiveCoordinates = (s.in.lhsMinusRhsAccumulatedConstraint == null)? new int [0] : DoubleMatrixND.sub2ind(coord, IntFactory1D.dense.make(new int[] { s.in.numConstraints, s.in.numDecVariables })).toArray();
		...
		Pointer nlp = g.CreateIpoptProblem(n, x_L, x_U, m, g_L, g_U, nele_jac, nele_hess, index_style, eval_f, eval_g, eval_grad_f, eval_jac_g, eval_h);
