...
/** @author Pablo */
class _OPERATOR_PLUS extends Expression
{
	...
	_OPERATOR_PLUS(OptimizationProblem model, Expression a, Expression b)
	{
		super(model);
		
		/* compute size */
		if (!Arrays.equals(a.getSize() , b.getSize())) throw new JOMException ("Operator '+'. Size mismatch: a and b should be of the same size, or one of them should be a scalar expression");
		this.resize(a.getSize());

		this.isLinear = (a.isLinear() && b.isLinear());
		this.affineExp = (isLinear)? a.getAffineExpression().operator_plus(b.getAffineExpression()) : null;
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		a_value.assign (b_value , DoubleFunctions.plus);
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return a_gradient.assign(b_gradient , DoubleFunctions.plus);
	}
