...
class _OPERATOR_E2EDIVIDE extends Expression
{
	...
	_OPERATOR_E2EDIVIDE(OptimizationProblem model, Expression a, Expression b)
	{
		...
		/* compute size */
		if (!Arrays.equals(a.getSize(), b.getSize())) throw new JOMException("Operator '/ or ./' (element-by-element division): Wrong size of "
				+ "array");
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		DoubleMatrix2D aux_3 = DoubleFactory2D.dense.repeat(b_value.assign(DoubleFunctions.square).reshape(b.getNumScalarExpressions(), 1), 1,
				valuesDVs.length);
