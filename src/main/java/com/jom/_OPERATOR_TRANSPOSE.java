...
class _OPERATOR_TRANSPOSE extends Expression
{
	...
	_OPERATOR_TRANSPOSE(OptimizationProblem model, Expression a)
	{
		...
		this.permutationMatrix = (this.isLinear())? null : computePermutationMatrix (a.getSize());; // if linear do not store the expressions. This info is already in the linear coefs matrix
	}
