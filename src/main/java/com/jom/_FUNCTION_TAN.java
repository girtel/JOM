...
class _FUNCTION_TAN extends Expression
{
	private final Expression a; 
	private final boolean isConstant;
	private final boolean isLinear;
	
	_FUNCTION_TAN(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0 ; cont < data.length ; cont ++) data [cont] = Math.tan(data [cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		return new DoubleMatrixND (this.getSize () , aValue.assign(DoubleFunctions.tan));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return  DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.cos).assign(DoubleFunctions.square).assign(DoubleFunctions.inv)).zMult(a_jacobian, null);
	}
