...
class _FUNCTION_EXP extends Expression
{
	private final Expression a;
	private final boolean    isConstant;
	private final boolean    isLinear;

	_FUNCTION_EXP(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0; cont < data.length; cont++) data[cont] = Math.exp(data[cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		return new DoubleMatrixND(this.getSize(), aValue.assign(DoubleFunctions.exp));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.exp)).zMult(a_jacobian, null);
