...
class _FUNCTION_ERLANGB_TRAF extends Expression
{
	...
	_FUNCTION_ERLANGB_TRAF(OptimizationProblem model, Expression loads , Expression servers)
	{
		...
		if (!Arrays.equals(servers.getSize(), loads.getSize())) throw new JOMException("Function 'erlangB': The array of number of servers and the array of number of loads must have the same size");
