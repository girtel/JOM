...
class _FUNCTION_ASIN extends Expression
{
	private final Expression a; 
	private final boolean isConstant;
	private final boolean isLinear;
	
	_FUNCTION_ASIN(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0 ; cont < data.length ; cont ++) data [cont] = Math.asin(data [cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		return new DoubleMatrixND (this.getSize () , aValue.assign(DoubleFunctions.asin));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return  DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.square).assign(DoubleFunctions.neg).assign(DoubleFunctions.plus(1)).assign(DoubleFunctions.sqrt).assign(DoubleFunctions.inv)).zMult(a_jacobian, null);
	}
	/* (non-Javadoc)
