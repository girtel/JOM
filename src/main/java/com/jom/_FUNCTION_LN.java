...
class _FUNCTION_LN extends Expression
{
	private final Expression a; 
	private final boolean isConstant;
	private final boolean isLinear;
	
	_FUNCTION_LN(OptimizationProblem model, Expression a)
	{
		super(model);
		...
		if (isLinear)
		{
			...
			for (int cont = 0 ; cont < data.length ; cont ++) data [cont] = Math.log(data [cont]);
	...
	DoubleMatrixND nl_evaluate(double[] valuesDVs)
	{
		...
		if (aValue.getMinLocation()[0] <= 0) throw new JOMException ("Function 'ln': Logarithms of non positive numbers are not defined");
		return new DoubleMatrixND (this.getSize () , aValue.assign(DoubleFunctions.log));
	}
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return  DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.inv)).zMult(a_jacobian, null);
	}
	/* (non-Javadoc)
