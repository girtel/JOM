...
class _OPERATOR_MULTIPLY extends Expression
{
	...
	_OPERATOR_MULTIPLY(OptimizationProblem model, Expression a, Expression b)
	{
		...
		/* compute size */
		if ((a.getNumDim() != 2) || (b.getNumDim() < 2)) throw new JOMException("Operator '*'  (matrix multiplication): Wrong size of the arrays. First array must be 2D, second array can have 2 or more dimensions");
		if ((a.getNumScalarExpressions() == 1) || (b.getNumScalarExpressions() == 1)) throw new JOMException("Operator '*' (matrix multiplication): Unexpected error. For scalar multiplication use .* operator");
		if (a.getSize() [1] != b.getSize() [0]) throw new JOMException("Operator '*'  (matrix multiplication): Wrong size of the arrays for multiplication. Size left matrix: " + Arrays.toString (a.getSize()) + ". Size right matrix: " + Arrays.toString (b.getSize()) );

		final int [] newSize = Arrays.copyOf (b.size , b.size.length); newSize [0] = a.size [0];
		this.resize(newSize);
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		for (int newIndex = 0 ; newIndex < this.numScalarExpressions ; newIndex ++)
		{
			...
			final int newRow = subindexesInRes.get (0); if (newRow != newIndex % a.size [0]) throw new RuntimeException ("Bad");
			final int newCol = subindexesInRes.get (1); 
			IntMatrix1D subIndexesInB = subindexesInRes.copy(); subIndexesInB.set(1, newCol);
		...
		while (b_it.hasNext())
		{
			...
			if ((!b_it.hasNext()) || (bCol != previousColumn_b) )
			{
				...
				for (Entry<Integer,HashSet<Integer>> aCellEntry : a.getActiveVarIds().entrySet())
				{
					...
					subindexes_res.set(0, resRow); subindexes_res.set(1, resCol);
					...
					HashSet<Integer> summand = ((HashSet<Integer>) aHs.clone()); summand.addAll(aCellColumnVector);
				...
				previousColumn_b = bCol; aColumn = new ArrayList<HashSet<Integer>> (bRows);
			}
