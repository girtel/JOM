...
class _FUNCTION_ACOS extends Expression
{
	...
	DoubleMatrix2D nl_evaluateJacobian(double[] valuesDVs)
	{
		...
		return DoubleFactory2D.sparse.diagonal(a_value.assign(DoubleFunctions.square).assign(DoubleFunctions.neg).assign(DoubleFunctions.plus(1))
				.assign(DoubleFunctions.sqrt).assign(DoubleFunctions.inv).assign(DoubleFunctions.neg)).zMult(a_jacobian, null);
	}
