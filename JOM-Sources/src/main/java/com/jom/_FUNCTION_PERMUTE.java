...
class _FUNCTION_PERMUTE extends Expression
{
	...
	_FUNCTION_PERMUTE(OptimizationProblem model, Expression a, Expression dimPermutation)
	{
		...
		if (!dimPermutation.isRowOrColumn2DExpression()) throw new JOMException("Function 'permute': the permutation must be a row or column "
				+ "vector");
		if (dimPermutation.getNumScalarExpressions() != a.getNumDim())
			throw new JOMException("Function 'permute': the number of elements in the permutation argument must be equal to the number of "
					+ "dimensions");
		...
		for (int oldDimensionIndex0 = 0; oldDimensionIndex0 < a.numDim; oldDimensionIndex0++)
		{
			...
			if ((newDimensionIndex0 < 0) || (newDimensionIndex0 >= a.numDim))
				throw new JOMException("Function 'permute': wrong dimension identifier");
		...
		this.permutationMatrix = (this.isLinear()) ?
				null :
				computePermutationMatrix(a.getSize(), newSize, permutationOfDimensionsIndex0); // if linear do not store the expressions. This info
		// is already in the linear coefs matrix
	}
	...
	private static DoubleMatrix2D computePermutationMatrix(int[] oldSize, int[] newSize, int[] permutationOfDimensionsIndex0)
	{
		...
		for (int oldCellIndex = 0; oldCellIndex < numScalarExpressions; oldCellIndex++)
		{
			...
			final IntMatrix1D newSubindexes = IntFactory1D.dense.make(newSize.length);
			for (int dim = 0; dim < newSize.length; dim++) newSubindexes.set(dim, oldSubindexes.get(permutationOfDimensionsIndex0[dim]));
	...
	LinkedHashMap<Integer, HashSet<Integer>> nl_getActiveVarIds()
	{
		...
		for (Entry<Integer, HashSet<Integer>> e : a.getActiveVarIds().entrySet())
		{
			...
			for (int dim = 0; dim < this.size.length; dim++) newSubindexes.set(dim, oldSubindexes.get(this.permutationOfDimensionsIndex0[dim]));
